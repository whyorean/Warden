    ...
    static class ViewHolder extends ListViewHolder {
        ...
        public void bindView(@NotNull ListItem item, @NotNull List<?> list) {
            if (item instanceof AppUsageItem) {
                App app = ((AppUsageItem) item).getApp();
                line1.setText(app.getDisplayName());
                if (app.getUsageStats() != null) {
                    line3.setText(StringUtils.joinWith(
                            " • ",
                            DateTimeUtil.millisToDay(app.getUsageStats().getLastTimeUsed()),
                            DateTimeUtil.millisToTime(app.getUsageStats().getLastTimeUsed())));
                    line2.setText(DateTimeUtil.millisToTimeDuration(app.getUsageStats().getTotalTimeInForeground()));
                }
