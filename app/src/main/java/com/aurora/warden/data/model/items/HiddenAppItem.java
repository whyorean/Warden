...
@Getter
@Setter
public class HiddenAppItem extends ListItem {

    private HiddenApp app;

    public HiddenAppItem(HiddenApp app) {
        this.app = app;
    }
    ...
    static class ViewHolder extends ListViewHolder {
        ...
        public void bindView(@NotNull ListItem item, @NotNull List<?> list) {
            if (item instanceof HiddenAppItem) {
                HiddenApp app = ((HiddenAppItem) item).getApp();
                line1.setText(app.getDisplayName());
                line2.setText(app.getPackageName());
                line3.setText(DateTimeUtil.millisToDay(app.getTimestamp()));
                img.setImageBitmap(ViewUtil.getBitmapFromRawString(app.getRawBitmap()));
            }
        }
